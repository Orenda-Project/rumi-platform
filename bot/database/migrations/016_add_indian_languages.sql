-- Migration: 016_add_indian_languages.sql
-- Purpose: Add 6 Indian languages (Hindi, Bengali, Marathi, Telugu, Indian Tamil, Kannada)
--          to the preferred_language CHECK constraint for India-market expansion.
-- Date: July 25, 2026
--
-- Context: The canonical fresh-install schema (infrastructure/supabase/00_complete-schema.sql)
-- has no CHECK on users.preferred_language, so fresh clones already accept these codes.
-- This migration only matters for deployments that previously applied 011_add_language_expansion.sql
-- (which added a restrictive CHECK). It is a no-op-safe drop + re-add.

-- Step 1: Drop existing constraint if present
ALTER TABLE users DROP CONSTRAINT IF EXISTS check_preferred_language;

-- Step 2: Re-add with the full union (PK regional + India + core).
