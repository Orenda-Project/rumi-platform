/**
 * Meta WhatsApp Cloud API channel driver — the production-tier driver.
 *
 * This is a MECHANICAL LIFT of bot/shared/services/whatsapp.service.js (which
 * is now a thin facade over messaging/index.js). The only intentional edits are
 * require-path depth fixes, since this file sits one directory deeper than the
 * original.
 *
 * MAINTENANCE: because it is a copy, it goes stale if the upstream service
 * changes. That already bit once — this branch was cut before
 * `feat(languages): add Indian-language support` landed on main, so the first
 * lift carried a hardcoded 10-language list while main had moved to the shared
 * config/supported-languages.js with region filtering. Re-lift from the CURRENT
 * whatsapp.service.js when rebasing, and let
 * tests/messaging/channel-driver-parity.test.js + the require-time assertion in
 * baileys-channel.service.js catch any method the Baileys driver hasn't caught
 * up with.
 */
