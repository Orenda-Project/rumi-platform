...
const STICKER_PATH = path.join(__dirname, '../marketing/Pedagogical Analysis.webp');

async function uploadPedagogicalAnimation() {
  try {
    console.log('╔═══════════════════════════════════════════════════════════════╗');
    console.log('║  UPLOAD PEDAGOGICAL ANALYSIS ANIMATION TO WHATSAPP            ║');
    console.log('╚═══════════════════════════════════════════════════════════════╝\n');
    ...
    if (!fs.existsSync(STICKER_PATH)) {
      console.error(`❌ Error: Pedagogical animation not found at: ${STICKER_PATH}`);
      process.exit(1);
    }
    ...
    console.log('\n⏳ Uploading pedagogical analysis animation to WhatsApp...\n');
    ...
    console.log(`      PEDAGOGICAL_ANALYSIS_MEDIA_ID=${mediaId}`);
    console.log('\n   2. Add to Railway environment variables (both bot and worker services):\n');
    console.log(`      PEDAGOGICAL_ANALYSIS_MEDIA_ID=${mediaId}`);
    console.log('\n✨ You can now use this media ID to send the pedagogical analysis animation!\n');
